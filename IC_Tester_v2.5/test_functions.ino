/*
 *  test_functions contains all IC related test routines
 *  
 *  author: Frank Hellmann, mail@gotohellmann.com
 *  
 */
...
void autoSearch(int pins)
{
  ...
  if (dataFile)
  {
    int count = 0;
    int i;
    for (i=0;i<20;i++) errpin[i] = false;
    i=0;

    drawStatus("Testing",GREEN);
    ...
    else
    {
      ...
      while (count--)
      {
        ...
        drawStatus(String(maxcount-count)+"/"+String(maxcount)+"  Touch->",GREEN);
        getTouch();
      }
      currentIC=ans[0].num;
      drawTop(ans[0].name);
      drawStatus(currentIC+" is OK",GREEN);
    }
  }
  else
  {
...
void manualSearch(String number)
{
  ...
  if (dataFile)
  {
    ...
    Serial.println("chipdb.txt is available");
    while (dataFile.available())
    {
      result = true;
      dataFile.readStringUntil('$');
